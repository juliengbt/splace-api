...
export default class AddressService {
  ...
  async exists(address: DeepPartial<Address>) : Promise<Address | undefined> {
    const query = this.getFullObjectQuery()
      .where('Address.id != :id', { id: address.id });

    if (address.locality) query.andWhere('LOWER(TRIM(Address.locality)) = LOWER(TRIM(:locality))', { locality: address.locality });
    if (address.locality === null) query.andWhere('Address.locality is NULL');

    if (address.district) query.andWhere('LOWER(TRIM(Address.district)) = LOWER(TRIM(:district))', { district: address.district });
    if (address.district === null) query.andWhere('Address.district is NULL');

    if (address.street_name) query.andWhere('LOWER(TRIM(Address.street_name)) = LOWER(TRIM(:street_name))', { street_name: address.street_name });
    if (address.street_name === null) query.andWhere('Address.street_name is NULL');

    if (address.street_num) query.andWhere('LOWER(TRIM(Address.street_num)) = LOWER(TRIM(:street_num))', { street_num: address.street_num });
    if (address.street_num === null) query.andWhere('Address.street_num is NULL');

    if (address.zipcode?.id) query.andWhere('Address.zipcode = :id_zipcode', { id_zipcode: address.zipcode.id });
